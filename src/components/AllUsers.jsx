  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        ...
        console.log("Fetched teams:", responseData);
        if (Array.isArray(responseData)) {
          setTeams(responseData); // Update teams state with the fetched data
        } else {
          console.error("Teams API did not return an array:", responseData);
          setTeams([]); // Ensure teams is always an array
        }
      } catch (error) {
        console.error("Error fetching teams data:", error);
        setTeams([]); // Handle error by setting teams to an empty array
      }
    };
