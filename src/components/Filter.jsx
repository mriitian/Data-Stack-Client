...
export default function Filter({ onChange }) {
  ...
  const fetchData = async () => {
    try {
      ...
      setDomain(responseData?.domain || []); // Fallback to empty array if undefined
      setGender(responseData?.gender || []); // Fallback to empty array if undefined
    } catch (error) {
      console.error("Error fetching data:", error);
      setDomain([]); // Ensure state is still valid even on error
      setGender([]);
    }
  };
